...
def TSP(Graph):
  last_key = Graph.keys()[0]
  InTree = []
  InTree.append(last_key)
  next_key = Graph[last_key][0]
  for i in range(len(Graph)):
    for u in Graph[last_key]:
      if u not in InTree and (s_distance(last_key,u) < s_distance(next_key,last_key)):
        next_key = u
    InTree.append(next_key)
    last_key = next_key
    for u in Graph[last_key]:
      if u not in InTree:
        next_key = u
...
##  for i in InTree:
##    sum = sum + i[1]
##  print "MST total distance ", sum, " miles"
##  
##  doubleTree = []
